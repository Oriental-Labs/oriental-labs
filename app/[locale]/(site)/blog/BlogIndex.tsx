...
function formatDate(dateStr: string | null, locale: Locale) {
  if (!dateStr) return '';
  return new Date(dateStr).toLocaleDateString(locale === 'es' ? 'es-UY' : 'en-US', {
    year: 'numeric',
    month: 'long',
    day: 'numeric',
  });
...
export function BlogIndex({ locale }: Props) {
  ...
  const filtered = posts.filter((p) => {
    ...
    const matchesQuery = !query || title.toLowerCase().includes(query.toLowerCase());
                ...
                >
                  ...
                  >
                    <div className="flex items-start justify-between gap-4">
                          ...
                          {post.tags.length > 0 && (
                            <div className="flex items-center gap-1.5 flex-wrap">
                              {post.tags.slice(0, 3).map((tag) => (
                                <span key={tag} className="px-2 py-0.5 rounded-md bg-slate-100 dark:bg-navy-700/60 text-slate-500 dark:text-slate-400 text-xs">
                                  {tag}
                                </span>
                              ))}
                            </div>
                      ...
                      <ArrowRight size={16} className="flex-shrink-0 text-slate-400 dark:text-slate-600 group-hover:text-electric-400 group-hover:translate-x-1 transition-all mt-1" />
                    </div>
                  </Link>
                </motion.div>
              );
